...
const clientClickedHandler = async (client: Client): Promise<void> => {
    ...
    try {
        const workspace = await window.io.workspaces.restoreWorkspace("Client Space", restoreConfig);
...
const raiseNotificationOnWorkspaceOpen = async (clientName: string, workspace: Workspace): Promise<void> => {
    ...
    const notification = await window.io.notifications.raise(options);
...
const start = async (): Promise<void> => {
    ...
    // TODO: Chapter 13
    const stocksAppHotkey: Hotkey = {
        hotkey: "alt+shift+s",
        description: "Starts the Stocks app."
    };

    const stocksAppHotkeyHandler = (): void => {
        io.appManager.application("stocks").start().catch(console.error);
    };

    const themeHotkey: Hotkey = {
        hotkey: "alt+shift+t",
        description: "Toggles the platform theme."
    };

    const themeHotkeyHandler = async (): Promise<void> => {
